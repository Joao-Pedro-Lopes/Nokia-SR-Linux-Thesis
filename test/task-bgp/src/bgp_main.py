...
for node_name, node_interfaces in interface_configs.items():
    ...
    filename = f'{node_name}_bgp.json'
    with open(f'output/bgp/{filename}', 'w') as f: # --> change to output/interfaces/{filename} done on test environment
        f.write(output)
        print(f'Wrote bgp configuration for {node_name} to {filename}')
